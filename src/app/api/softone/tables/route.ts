...
import { getSoftOneObjectTables } from "@/lib/softone-api";
import { decrypt } from "@/lib/encryption";
import { authenticateSoftOneAPI } from "@/lib/softone-api";

/**
 * POST /api/softone/tables
 * 
 * SERVER-SIDE API ROUTE - All SoftOne API calls are made server-side only.
 * 
 * Gets tables for a specific SoftOne object.
 * Requires connectionId or clientID, and objectName.
 * 
...
export async function POST(request: Request) {
  try {
    ...
    const { connectionId, clientID, objectName, appId, version } = body;
    ...
    // Get tables for the object with appId and version
    const result = await getSoftOneObjectTables(
      objectName,
      clientIdToUse,
      appIdToUse,
      version || "1"
    ...
    console.error("SoftOne get tables error:", error);
    return NextResponse.json(
      {
        success: false,
